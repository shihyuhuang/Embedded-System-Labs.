...
int main(){
    ...
    if (acc16 > UINT14_MAX/2)
        acc16 -= UINT14_MAX;
    in[0] = ((float)acc16) / 4096.0f;

    acc16 = (res[2] << 6) | (res[3] >> 2);
    if (acc16 > UINT14_MAX/2)
        acc16 -= UINT14_MAX;
    in[1] = ((float)acc16) / 4096.0f;

    acc16 = (res[4] << 6) | (res[5] >> 2);
    if (acc16 > UINT14_MAX/2)
        acc16 -= UINT14_MAX;
    in[2] = ((float)acc16) / 4096.0f;
    ...
    while(true) {
        GetAccelerometer();
        float a = (t[0] * in[0]) + (t[1] * in[1]) + (t[2] * in[2]);
        float cos_t = a/((sqrt(t[0] * t[0] + t[1] * t[1] + t[2] * t[2])) * (sqrt(in[0] * in[0] + in[1] * in[1] + in[2] * in[2])));
        float degree = acos(cos_t) * 180.0 / 3.1415;
...
void publish_message(MQTT::Client<MQTTNetwork, Countdown>* client) {
    ...
    char buff[100] = {0};
    sprintf(buff, "%1.4f %1.4f %1.4f %f", t[0], t[1], t[2], interval);
